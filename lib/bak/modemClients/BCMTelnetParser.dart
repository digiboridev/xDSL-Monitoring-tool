...
//             RegExp(r'(?<=Mode:                   ).+').firstMatch(msg).group(0);
//         var spd = RegExp(r'(?<=Bearer: 0,).+').firstMatch(msg).group(0);

//         lineData['upMaxRate'] =
//             RegExp(r'(?<=Upstream rate = )\d+').firstMatch(spd).group(0);
//         lineData['downMaxRate'] =
//             RegExp(r'(?<=Downstream rate = )\d+').firstMatch(spd).group(0);

//         var maxSpd = RegExp(r'(?<=Max:    ).+').firstMatch(msg).group(0);

//         lineData['upRate'] =
//             RegExp(r'(?<=Upstream rate = )\d+').firstMatch(maxSpd).group(0);
//         lineData['downRate'] =
//             RegExp(r'(?<=Downstream rate = )\d+').firstMatch(maxSpd).group(0);

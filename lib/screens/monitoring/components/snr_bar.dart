                  ...
                  RepaintBoundary(
                    child: Builder(
                      builder: (context) {
                        ...
                        if (showRecent) {
                          final minRecent = context.select<MonitoringScreenViewModel, int?>((s) => s.recentCounters?.downSNRMin);
                          final maxRecent = context.select<MonitoringScreenViewModel, int?>((s) => s.recentCounters?.downSNRMax);
                          final avgRecent = context.select<MonitoringScreenViewModel, int?>((s) => s.recentCounters?.downSNRAvg);

                          return Column(
                            children: [
                              MinMaxAvgRow(min: minRecent, max: maxRecent, avg: avgRecent, prefix: 'recent'),
                              MinMaxAvgRow(min: min, max: max, avg: avg, prefix: 'total'),
                            ],
                          );
                        }
                  ...
                  RepaintBoundary(
                    child: Builder(
                      builder: (context) {
                        ...
                        final showRecent = context.select<MonitoringScreenViewModel, bool>((s) => s.recentCounters?.maxCountReached ?? false);
                        if (showRecent) {
                          final minRecent = context.select<MonitoringScreenViewModel, int?>((s) => s.recentCounters?.upSNRMin);
                          final maxRecent = context.select<MonitoringScreenViewModel, int?>((s) => s.recentCounters?.upSNRMax);
                          final avgRecent = context.select<MonitoringScreenViewModel, int?>((s) => s.recentCounters?.upSNRAvg);
                  ...
                  RepaintBoundary(
                    child: Builder(
                      builder: (context) {
                        ...
                        final showRecent = context.select<MonitoringScreenViewModel, bool>((s) => s.recentCounters?.maxCountReached ?? false);
                        if (showRecent) {
                          final minRecent = context.select<MonitoringScreenViewModel, int?>((s) => s.recentCounters?.downATTNMin);
                          final maxRecent = context.select<MonitoringScreenViewModel, int?>((s) => s.recentCounters?.downATTNMax);
                          final avgRecent = context.select<MonitoringScreenViewModel, int?>((s) => s.recentCounters?.downATTNAvg);
                  ...
                  RepaintBoundary(
                    child: Builder(
                      builder: (context) {
                        ...
                        final showRecent = context.select<MonitoringScreenViewModel, bool>((s) => s.recentCounters?.maxCountReached ?? false);
                        if (showRecent) {
                          final minRecent = context.select<MonitoringScreenViewModel, int?>((s) => s.recentCounters?.upATTNMin);
                          final maxRecent = context.select<MonitoringScreenViewModel, int?>((s) => s.recentCounters?.upATTNMax);
                          final avgRecent = context.select<MonitoringScreenViewModel, int?>((s) => s.recentCounters?.upATTNAvg);
...
class MinMaxAvgRow extends StatelessWidget {
  ...
  final int? avg;
  final String? prefix;
  const MinMaxAvgRow({super.key, this.min, this.max, this.avg, this.prefix});
  ...
  Widget build(BuildContext context) {
    return Row(
      mainAxisAlignment: MainAxisAlignment.center,
      children: [
        if (prefix != null) Text('$prefix', style: TextStyles.f8.blueGrey800),

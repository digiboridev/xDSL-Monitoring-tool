...
class BarChartPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int i = 0; i < increasedData.length; i++) {
      ...
      final double scale = val / max;
      final double x = i * step;
      final double y = scale * size.height;
      canvas.drawLine(Offset(x, size.height / 2 + y / 2), Offset(x, size.height / 2 - y / 2), _barPaint..strokeWidth = step);
    }
  }

  Paint get _barPaint => Paint()..color = Colors.blueGrey.shade800;
